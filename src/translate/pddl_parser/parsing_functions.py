...
def parse_domain_custom(checker):
    ...
    predicates = [[pred] + checker.predicates[pred]['params'] for pred in
                  checker.predicates]
    predicates = [parse_predicate(entry) for entry in predicates]
    predicates += [pddl.Predicate("=", [pddl.TypedObject("?x", "object"),
    ...
    actions = []
    entries = [[':action', action, ':parameters', checker.actions[action]['params'],
                ':precondition', checker.actions[action]['logic_precs'], ':effect',
                checker.actions[action]['logic_effs']] for action in checker.actions]
